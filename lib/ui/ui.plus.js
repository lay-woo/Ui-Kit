...
function JState(e,prms){
    ...
    _S.of=function(a,event){
        ...
        var blur=function(event){
            var tg=event.target;
            if((a!=tg && !J(a).find(tg).get(0)) && (nexus!=tg && !J(nexus).find(tg).get(0))){
...
Ui.ready(function(){
    ...
    if(window['JRegion']){
        var region=new JRegion();
        J(document).on({
            'blur':function(){},
            'focus':function(){
                var _po_region=new JPopuper({modal:'element',get:region.e,scrolling:'no',classname:'',width:'auto',height:'auto'});
                _po_region.open();
            }
        },"input[region]");
    }
