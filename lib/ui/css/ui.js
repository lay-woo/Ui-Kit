...
function objectProperty(o,p,d){var t=typeof(o);if(t!='object' && t!='function'){return undefined;}if(!p || typeof(p)!=="string"){return o}p=p.replace(/^\./,'');var ps=p.split("."),c=o;for(var i=0,l=ps.length;i<l;i++){if(c && c.hasOwnProperty(ps[i])){c=c[ps[i]];}else{return d||undefined;}}return c;}
...
function isSelecter(x){if(_typeof(x)==="string"){var s=x.charAt(0);if(s==="."||s==="#"){return true;}};return false;}
...
(!window.locations) && (window.locations=Ui.locations=Ui.location=new JLocations());
...
function JScroller(e){
	...
	var scrollbar=function(dir){
		...
		_s.je=J('<s class="j_scrollbar j_scrollbar_'+dir+'" style="display:block;position:absolute;width:0px;height:0px;visibility:hidden;"><s class="scrollbar"><b class="handle"></b></s></s>').appendTo(J(_S.e).parent());
		_s.e=_s.je[0];
		_s.bar=_s.je.find('.scrollbar').get(0),
			_s.handle=_s.je.find('.handle').get(0);
		...
		_s.active=function(of){of?_s.e.style.visibility="":_s.e.style.visibility="hidden";};
	...
	initialize:function(optns){
		...
		J(document).on({
			'mouseover':function(event){event.preventDefault();
				...
				var ops=__getOptions(this,_default);
		...
		function __getOptions(trg,_prms){
				if(!trg){return J.extend({},_prms);}
				var _E=E(trg),_ops=J.extend({},_prms,_E.options({"get":_prms.get,"title":_prms.title,"alt":"","modal":_prms.modal}));
				_ops.trg=trg;
				!_E.has('ops') && _E.set("ops",{});
				if(!_E.ops["get"]){
					if(_E.ops["get"]!==null && isSelecter(_ops.get)){
						var _e=E(trg).near(_E.attribute('rsp')||_ops.get,0);
						//var _e=J(Ui.gRsp(trg,_E.attribute('rsp')||_ops.get,0)).get(0);
						_E.ops.get=isE(_e)?_e:null;
						_E.ops.modal='element';
					}
					if(!_E.ops["get"] && (_ops.title || _ops.alt)){
						_E.ops.get='<span class="text">'+(_ops.title||_ops.alt)+'</span>';
						_E.ops.modal='html';
						_E.ops.title="";
...
Ui("popup",{
	domready:false,
	topwindow:true,
	initialize:function(_prms){
		J.extend(_prms,{win:_prms.win||Ui.wintop,self:window.self});
		var Pop=this;
		function assign(po,optns){
			if(!optns.trg){return;}
			J(document).on('click',optns.trg,function(event){
				optns.event=event;
				optns.self=window.self;
				E(this).popuper=po.open(optns,this);
				event.preventDefault();
				return false;
			});
		}
		if(_prms.win!=_prms.self){if(!_prms.win['Ui']){_prms.win['Ui']=Ui}}
		Pop=new Ui.ui.popup.pop(_prms);
		assign(Pop,_prms);

		return Pop;
	},
	pop:function(_prms){
		var _PO=function(_ops){
				var _S=this;
		...
		_PO.prototype.__getOptions=function(trg,optns){
			if(!trg){return J.extend({},this.options,optns);}
			var _E=E(trg),_ops=J.extend({},this.options,optns,_E.options());
			_ops.trg=trg;
		...
		_PO.prototype.current=function(){
		...
		_PO.prototype.frame=function(){return _S.popup.frame;}
		_PO.prototype.open=function(ops,trg){
            		var _popuper=this.create(ops,trg);
				_popuper && _popuper.open();
			return _popuper;
		};
        _PO.prototype.create=function(ops,trg){
            var _S=this,trg=trg||ops.trg;
            if(trg){ops=this.__getOptions(trg,ops);}else{ops.trg='';}
		...
		_PO.prototype.close=function(prm){
			var _s=this;
			if(!prm){
		...
		_PO.prototype.get=function(key){
			if(!key){
				return this.popup;
			}else{
		...
		_PO.prototype.point=function(pointo){
		...
		return new _PO(_prms);
	}
});

Ui.popuper=Ui({fn:'popup',key:'popuper',trg:'.b_pop',modal:'ajax',mask:true,width:"auto",height:"auto",maskerOpacity:0.8,offset:0});
Ui({fn:'tips',key:'po',trg:'.b_tp',modal:'element',mode:'po',get:".j_poc",win:Ui.topwin,delayin:200,delayout:350,masker:false,maskerOpacity:0,blur:true,stay:true});
Ui({fn:'popup',key:'pop',trg:'.b_po',modal:'element',mode:'po',get:".j_poc",win:window,mask:true,blur:true,position:"align",width:"auto",height:"auto",maskerOpacity:0.15,offset:14});
